...
import com.example.musicapp.service.MyApplication.Companion.CHANEL_ID
...
class MyService : Service() {
    ...
    companion object{
        ...
        val ACTION_SEEK_BAR: Int = 6
        val ACTION_NO_REPEAT: Int = 7
        val ACTION_REPEAT: Int = 8
        val ACTION_REPEAT_ONE_SONG: Int = 9
        val ACTION_SHUFFLE: Int = 10
        val ACTION_NO_SHUFFLE: Int = 11
        val ACTION_STOP: Int = 12
        val ACTION_GET_INFOR_FOR_PLAY_ACTIVITY : Int =13
        val ACTION_UPDATE_CURRENT_TIME: Int = 99
...
//        var uri: Uri = Uri.parse("https://mp3-s1-m-zmp3.zadn.vn/ffab9ce685a76cf935b6/2885442504735776385?authen=exp=1637471274~acl=/ffab9ce685a76cf935b6/*~hmac=6735c0bc7df7f71b8637b368b4336fca&fs=MTYzNzI5ODQ3NDk1N3x3ZWJWNHwxLjU0LjE5OS4yNDmUsIC")
    ...
    private fun handleActionMusic(action: Int){
        when(action){
            ACTION_STOP ->{
                sendActionToMainActivity(ACTION_STOP)
                sendActionToActivity(ACTION_STOP)
                request.cancel()
//                sendStopServiceToActivity(ACTION_STOP)
                stopSelf()
            }
            ACTION_PAUSE ->{
                pauseMusic()
            ...
            ACTION_GET_INFOR_FOR_PLAY_ACTIVITY ->{
                sendActionToActivity(ACTION_GET_INFOR_FOR_PLAY_ACTIVITY)
            }
        }
    }
    ...
    private fun sendNotification(song: Song) {
        ...
        remoteViews.setImageViewResource(
            R.id.noti_bt_play_pause,
            R.drawable.baseline_pause_black_20
        )
        remoteViews.setImageViewResource(
            R.id.noti_bt_next_song,
            R.drawable.baseline_skip_next_black_20
        )
        remoteViews.setImageViewResource(
            R.id.noti_bt_previous_song,
            R.drawable.baseline_skip_previous_black_20
        )
        ...
        if(isPlaying){
            ...
            remoteViews.setImageViewResource(
                R.id.noti_bt_play_pause,
                R.drawable.baseline_play_arrow_black_20
            )
        }
...
//        intent.putExtra("action_music", ACTION_OPEN_PLAY_ACTIVITY_FROM_MAIN)
//        val pendingIntent: PendingIntent
//
//        pendingIntent = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
//            PendingIntent.getBroadcast(
//                applicationContext, ACTION_OPEN_PLAY_ACTIVITY_FROM_MAIN,
...
//        }
    ...
    private fun getPendingIntent(context: Context, action: Int): PendingIntent{
        ...
        return PendingIntent.getBroadcast(context.applicationContext, action, intent, PendingIntent.FLAG_MUTABLE)
    }
